...
* Second Mode
	...
	2. params - (`mixed`) The query string to apply to the new hash URI. This can either be a literal query string or an object comprized of key/value pairs. (ex. 'hello=world&hello2=2')

* Third Mode
	1. prefix - (`string`) The hash prefix to navigate to. (ex. '#')
	2. page - (`string`) The page/state designator to navigate to. (ex. 'home')
	3. params - (`mixed`) The query string to apply to the new hash URI. This can either be a literal query string or an object comprized of key/value pairs.  (ex. 'hello=world&hello2=2')
...
####Returns
* (`boolean`) `true` if the whole unserialization process succeeded, else `false`. (Note that `false` does not necessarily mean the unserialization process was a total failure)
